...
<div class="w-full max-w-md" data-testid="login-page-state">
  <div class="card px-6 py-7 sm:px-8 sm:py-8" data-testid="login-card-state">
    ...
    {#if errorMsg}
      <div class="auth-error-box mb-4 rounded-lg border px-3 py-2 text-sm" role="alert" data-testid="login-error-state">
        {errorMsg}
      </div>
    {/if}
  <form
  ...
  >
      <div class="space-y-2">
        ...
        <input
          ...
          data-testid="login-email-input-action"
          on:input={() => {
            emailError = null;
            errorMsg = null;
          }}
        />
        {#if emailError}
          <p class="auth-error-text text-xs" data-testid="login-email-error-state">{emailError}</p>
        {/if}
      </div>
      <div class="space-y-2">
        ...
        <div class="relative">
          <input
            ...
            data-testid="login-password-input-action"
            on:input={() => {
              passwordError = null;
              errorMsg = null;
            }}
        ...
        {#if passwordError}
          <p class="auth-error-text text-xs" data-testid="login-password-error-state">{passwordError}</p>
        {/if}
      </div>
      <button
        type="submit"
